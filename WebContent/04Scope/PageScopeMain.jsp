...
<body>
	...
	<%
	/*
	page영역에 저장된 속성을 읽어옴. 영역에 저장될때는 Object형으로 자동형변환 되므로, 읽어올때도 Object형으로
	가져오게 된다.
	*/
	...
	*/
	...
	String m1Str = String.format("아이디:%s, 비번:%s, 이름:%s, 가입월:%s", m1.getId(), m1.getPass(), m1.getName(), m1.getRegidate());
	...
	<ul>
		...
		<li>MemberDTO타입2 : 아이디:<%=m2.getId()%>, 비번:<%=m2.getPass()%>, 이름:<%=m2.getName()%>,
			가입월:<%=m2.getRegidate()%></li>
	</ul>
	<h2>페이지 이동</h2>
